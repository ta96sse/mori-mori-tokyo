...
> **エージェント構成（2026年現在）**
> - **Antigravity**（Google DeepMind）→ オーケストレーター / リサーチャー / ビジュアルベリファイア
> - **Claude Code**（Anthropic）→ メインワーカー（コード実装・長時間セッション）
>
> Antigravity の詳細設定 → `GEMINI.md`
> Claude Code の詳細設定 → `CLAUDE.md`
...
|------|-------------|-------------|
| コード実装・編集 | ✅ | ✅ |
| ブラウザ操作・表示確認 | ✅ `browser_subagent` | ❌ |
| 画像生成（UIモック等） | ✅ `generate_image` | ❌ |
| Web検索（最新情報調査） | ✅ `search_web` | ❌ |
| ターミナルコマンド実行 | ✅（ユーザー承認あり） | ✅（自動実行） |
| 長時間コーディングセッション | △ | ✅ |
| `yarn astro check` 修正ループ | △ | ✅ |
...
- ライブラリ・ベスプラ調査
- タスク分解とオーケストレーション設計
- ドキュメント整備（CLAUDE.md / AGENTS.md / GEMINI.md）
- UIモックアップ生成
...
## Antigravityのワークフローテンプレート

`.antigravity/commands/` にテンプレートがあります。

| ファイル | 用途 |
|---------|------|
| `orchestrate.md` | マルチエージェントタスクの統括フロー |
| `delegate.md` | Claude Codeへの委任フォーマット |
| `review.md` | PRコードレビューチェックリスト |
| `visual-check.md` | ブラウザ表示確認手順 |
| `research.md` | リサーチタスクテンプレート |
